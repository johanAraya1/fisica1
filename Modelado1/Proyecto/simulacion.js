...
function draw() {
    ...
    if (y >= canvas.height - 20 && !hit) {
        ...
        ctx.fillStyle = "red";
        ctx.font = "20px Arial";
        ctx.fillText("Falló", canvas.width / 2 - 20, canvas.height / 2);
    }
...
// Función  para hacer que los objetivos palpiten suavemente
function drawTriangle(px, py, size) {
    let pulsatingSize = size + Math.sin(Date.now() * 0.005) * 5; // Hace que palpite suavemente
    ctx.beginPath();
...
function nuevoJuego() {
    ...
    // Restablecer la posición inicial del proyectil
    x = startX;
    y = startY;
    ...
    // Limpiar la pantalla y redibujar todo
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage(backgroundImg, 0, 0, canvas.width, canvas.height);

    // Volver a mostrar los objetivos
    targets.forEach(target => drawTarget(target.x, target.y, target.size));

    // Restablecer la información en pantalla
    document.getElementById("info").innerHTML = `
        Tiempo: 0.00 s <br>
        Posición X: 0.00 m <br>
        Posición Y: 0.00 m <br>
        Velocidad X: 0.00 m/s <br>
        Velocidad Y: 0.00 m/s <br>
    `;

    // Detener la animación del proyectil si estaba en curso
